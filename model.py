...
class MRNet(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Linear(256, 1) #and it outputs 1 value for binary classification (e.g., normal or abnormal).
    ...
    def forward(self, x):
        ...
        x = self.classifier(x) #Passes the pooled features through the final classification layer to produce the output.
        return x

class TripleMRNet(nn.Module): #combines features from three different views of MRI data (axial, sagittal, and coronal).
    def __init__(self, backbone="resnet18", training=True): #This initializes the model, with the option to choose either ResNet18 or AlexNet as the backbone network for each MRI view (axial, sagittal, and coronal).
        super().__init__()
